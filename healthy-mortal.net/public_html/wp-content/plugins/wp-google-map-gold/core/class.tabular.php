			...
			if ( ! empty( $this->currenttimestamp_field ) && $column_name == $this->currenttimestamp_field ) {
				...
				foreach ( $this->actions as $action ) {
					...
					if ( 'delete' == $action_slug ) {
						$actions[ $action_slug ] = sprintf( '<a href="#" data-item-id="' . $item->{$this->primary_col} . '" data-page-slug="'.$this->admin_listing_page_name.'" data-record-type="'.$this->primary_col.'" data-action="delete" data-toggle="modal" data-target="#delete_fc_record">' . $action_label . '<span class="delete_icon"></span></a>', $item->{$this->primary_col} );
